...
class RoundSTE(torch.autograd.Function):
    """
    Bypasses the non-differentiable nature of torch.round().
    """
    @staticmethod
    def forward(ctx, x):
    ...
    def backward(ctx, grad_output):
        # The gradient flows backwards unimpeded
        return grad_output, None

# Helper function to easily call the STE
def round_ste(x):
    return RoundSTE.apply(x)

class GradScaler(torch.autograd.Function):
    """
    Scales the gradient for the step size (scale parameter) to prevent explosive updates 
    ...
    @staticmethod
    def backward(ctx, grad_output):
        # Scale the gradient down by the calculated factor
        return grad_output * ctx.scale_factor, None

def scale_grad(x, scale_factor):
    return GradScaler.apply(x, scale_factor)
