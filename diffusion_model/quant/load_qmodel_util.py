...
def get_qmodel(model_type, pipe, ckpt_path, 
    ...
    if model_type == 'sd':
        ...
        cali_data = (
            torch.randn(1, 4096, 128, dtype=torch.bfloat16),  # hidden_states
            torch.randn(1, 512, 7680, dtype=torch.bfloat16),  # encoder_hidden_states
            torch.randint(0, 1000, (1,), dtype=torch.float32),# timestep
            torch.zeros(1, 4096, 4),
            torch.zeros(1, 512, 4),
        )
    else:
