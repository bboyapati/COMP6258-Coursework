...
def collect_data(model_type, pipe, coco_path, cali_data_path, cali_prompt_data_n=64, step_size=-1, callback_keys=[]):
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        if model_type == "sdxl":
            ...
            pipe(prompt=prompts_batch, num_inference_steps=step_size,
                guidance_scale=1.0,
                callback_on_step_end=callback_on_step_end,
                callback_on_step_end_tensor_inputs=callback_keys)
        else:
...
def cali_data_preprocessing(model_type, 
    ...
    if model_type == "sdxl":
        ...
        cali_data = (
            torch.cat(cali_data['latents'], dim=0).to(dtype=dtype),
            torch.cat(cali_data['prompt_embeds'], dim=0).to(dtype=dtype),
            torch.cat(timesteps, dim=0),
            torch.cat(cali_data['latent_image_ids'], dim=0),
            torch.cat(cali_data['text_ids'], dim=0),
            )
    else:
        cali_data = (torch.cat(cali_data["latent_model_input"], dim=0), 
                    torch.cat(timesteps, dim=0),
...
def calibration_data_generation(model_type, 
    ...
    cali_data, interval = cali_data_preprocessing(model_type, cali_data, cali_data_size, step_size, coco_path, cali_prompt_data_n)

    if not time_aware_aqtizer:
        interval = cali_data[0].shape[0]

    if model_type == "flux2":
        cali_data = tuple(map(lambda x: x.to("cpu"), cali_data))
    else:
