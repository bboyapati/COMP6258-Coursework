...
def parse_args():
    ...
    parser.add_argument('--prompt', type=str, default="A majestic mountain range with snow-capped peaks rising high into the sky, surrounded by a lush green forest at its base, under a clear blue sky with a few wispy clouds floating by. The scene is bathed in the golden light of the setting sun, casting long shadows and a warm glow over the landscape, creating a serene and breathtaking view.", help='Test prompt path')
    # common setting
...
def inference(model_type, pipe, prompt, precision, num_inference_steps, seed=42):
    ...
    if model_type == "sdxl":
        ...
        images = pipe(prompt=[prompt]*2, num_inference_steps=num_inference_steps, guidance_scale=1.0)[0]
    else:
        images = pipe([prompt]*2, num_inference_steps=num_inference_steps)[0]
    for i in range(len(images)):
        prompt_name = prompt.replace(' ', '_')
        images[i].save(f"tmp_{model_type}_{prompt_name[:min(len(prompt_name), 20)]}_{i}_{precision}.png")
...
def main():
    ...
    # inference fp model
    inference(MODEL_TYPE, pipe, opt.prompt, 'bf' if MODEL_TYPE == "flux2" else 'fp', num_inference_steps)
    ...
    if hasattr(pipe, "unet"):
        ...
        # pipe.transformer.disable_out_quantization()
